...
#' the count of patients who have the concept identified in the list element.
#'
...
check_ecp <- function(ecp_list){
  ...
  for(i in 1:length(ecp_list)){
    ...
    concept_group <- ecp_list[[i]][[3]] %>% select(concept_group) %>% distinct() %>% pull()
    
    message(paste0('Starting ', concept_group))
    
    total_pts <- ecp_list[[i]][[1]] %>%
      summarise(total_pt_ct = n_distinct(person_id)) %>%
      collect()
    
    total_pop <- site_cdm_tbl('person') %>%
      summarise(total_pop = n_distinct(person_id)) %>%
      collect()
    
    join_cols <- set_names('concept_id', ecp_list[[i]][[2]])
    
    fact_pts <- ecp_list[[i]][[1]] %>%
      inner_join(ecp_list[[i]][[3]], by = join_cols) %>%
    ...
    final_tbl <- total_pts %>%
      mutate(total_pop = total_pop$total_pop,
             ...
             check_name = ecp_list[[i]][[4]]) %>%
      add_meta(check_lib = 'ecp') 
    
    
    result[[paste0(ecp_list[[i]][[4]])]] <- final_tbl
  }
