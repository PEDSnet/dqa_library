...
compute_st_cons_dc_meta_tbl <- function(meta_tbls, 
                            #           versions_tbl_list,
...
check_st_cons_dc <- function(prev_v_tbls,
                             current_v_tbls,
                             meta_tbls,
  ...
  for(i in 1:length(prev_v_tbls)) {
    ...
    message(paste0('Computing previous version ',names(prev_v_tbls[i])))
    ...
    prev_v_cts[[names(prev_v_tbls[i])]] <- this_round %>% 
      mutate(database_version=prev_v) %>% add_meta(check_lib=check_string,
                                                   version=prev_v)
  ...
  for(i in 1:length(current_v_tbls)) {
    ...
    message(paste0('Computing current version ',names(prev_v_tbls[i])))
    ...
    current_v_cts[[names(current_v_tbls[i])]] <- this_round %>% 
      mutate(database_version=current_v) %>% add_meta(check_lib=check_string)
  ...
  meta <- compute_st_cons_dc_meta_tbl(meta_tbls=meta_tbls,
                           #versions_tbl_list=combined_list,
                           check_string=check_string)
  
  combined_list[[paste0(check_string,'_meta')]] <- meta
