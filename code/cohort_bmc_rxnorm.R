...
check_bmc_rxnorm <- function(drug_tbl_list_args,
                             check_string='bmc_rxnorm') {
  ...
  for(i in 1:length(drug_tbl_list_args)) {
    ...
    if(length(rxnorm_grpd) > 1) {
      ...
      props <- 
        ...
        mutate(check_name = drug_tbl_list_args[[i]][[3]]) %>%
        mutate(check_desc = drug_tbl_list_args[[i]][[2]]) %>%
        mutate(check_desc_short =drug_tbl_name)
